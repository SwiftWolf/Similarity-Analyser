...
public class FibHeap {
	...
	private static class FibHeapNode {

		private Object object;
		private int key;
		private FibHeapNode parent;
		private FibHeapNode leftSibling;
		private FibHeapNode rightSibling;
		private FibHeapNode child;

		// mark indicates wheter this node has lost a child since the last time
		...
		// A number that indicates when this node was inserted into the heap.
		// Added by me.
		// The minimum node in the heap is the node with the smallest key. If we
		// have two or more node with the smallest key, then
		// the min node is the node with the smallest key and the smallest
		// insertNum. The nodes that are added to the heap are the
		// nodes in B in BipartiteMatching.java. We always add the nodes that
		// are closest to a node a in A first. By using insertNum
		// then we do not need to do unordered matchings of the nodes in A and B
	...
	private HashMap<Object, FibHeapNode> objectToNode;
	// Number of nodes in the heap
	private int numberOfNodes;
	// Count how many nodes that have been added. Not the same as numberOfNodes
	// since nodes can have been removed from the heap.
	...
	public FibHeap(int maxSize) {
		...
		numberOfNodes = 0;
		insertNumCounter = 0;
		objectToNode = new HashMap<Object, FibHeapNode>(maxSize);

	}

	public void insert(Object object, int key) {
		// Reset counter if number of nodes is zero, because then we start over
		// again.
		if (numberOfNodes == 0) {
			insertNumCounter = 0;
		}
	...
	public Object extractMin() {
		...
		if (z != null) {
			...
			// else set the rightSibling of min as the new min node and run a
			// consolidation on the heap
			else {
				min = z.rightSibling;
				consolidate();
			}
	...
	public void consolidate() {
		...
		// Array can contain trees of different sizes. We can have size trees of
		// the
		...
		while (numRoots > 0) {
			...
			// The degree of x
			int d = x.degree;
			// Since we might move x we set its rightSibling as the next node
			// that we will process
			FibHeapNode next = x.rightSibling;

			// is there a subtree with the same size in the root list, then make
			// a bigger tree
			while (array[d] != null) {
				FibHeapNode y = array[d];

				// if x has a bigger key than y, or if the keys are equal but x
				// has a bigger insertNum, then
				...
				// there is no node with this degree anymore. we increase the
				// degree and see if we can build an even larger tree
				array[d] = null;
				d++;
			}
		...
		for (int i = 0; i <= size; i++) {
			if (array[i] != null) {
				// array[i] is the new min node if min = null or if it has a
				// smaller key than min or if its key equals min and insertNum
				// of array[i] is smaller than
				// the insertNum of min.
				if ((min == null) || (array[i].key < min.key)
						|| ((array[i].key == min.key) && (array[i].insertNum < min.insertNum))) {
					min = array[i];
				}
	...
	// used when we remove child from the root list and make it a child of
	// parent
	...
	public void decreaseKey(Object object, int key) {
		...
		if (objectToNode.containsKey(object)) {
			...
			// find the node
			FibHeapNode child = objectToNode.get(object);
...
//-
			...
			// perform cut and cascading cut if parent != null and child has a
			// lower key than parent or they have an equal key but child
	...
	public void cascadingCut(FibHeapNode node) {
		...
		if (parent != null) {
			...
			// The next time we remove a child from node then node will be cut
			// from its parent.
			if (node.mark == false) {
				node.mark = true;
			}
	...
	// delete a node. the node is deleted by decreasing its key to
	// Integer.MIN_VALUE and then by using extractMin
